...
from megatron.core.models.multimodal.mini_retro_model import IGNORE_INDEX, IMAGE_TOKEN, VIDEO_TOKEN
...
class TaskEncoder(DefaultTaskEncoder[OCRSample, OCRSample, ImageTaskBatchPacked, dict]):
    ...
    def encode_sample(self, sample: Union[CaptioningSample, OCRSample, VQASample, SimilarityInterleavedSample]):
        ...
        if isinstance(sample, OCRSample):
            ...
            is_mini_retro_training = sample.__subflavors__["is_mini_retro_training"] \
                if "is_mini_retro_training" in sample.__subflavors__ else False

            if "mini-retro" in sample.__key__ or is_mini_retro_training:
                yield self.encode_mini_retro_pretrain(sample)
            else:
                yield self.encode_any_single_turn_vqa(sample)
        elif isinstance(sample, SimilarityInterleavedSample):
            yield self.encode_mini_retro_sft(sample)
    ...
    def encode_mini_retro_pretrain(self, sample: VQASample):
        """Encode pretrain sample in MINI_RETRO style."""
        ...
        # MINI_RETRO training: override text-prompt with just the image.
        conv = [
            # Note: no system message.
    ...
    def encode_sample_list(self, samples: SampleListSample):
        """We encode the list of samples using encode_mini_retro_sft on each sample."""
        ...
        for idx, sample in enumerate(samples.samples):
            try:
                encoded_sample = self.encode_mini_retro_sft(sample, truncate_for_sample_list_packing=True)
    ...
    def encode_mini_retro_sft(self,
