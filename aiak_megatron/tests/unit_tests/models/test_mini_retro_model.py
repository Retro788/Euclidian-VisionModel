...
from megatron.core.models.multimodal.mini_retro_model import MiniRetroModel
...
class TestMiniRetroModel:
    ...
    def test_constructor(self):
        assert isinstance(self.model, MiniRetroModel)
...
class TestMiniRetroModelSigLIP:
    @pytest.mark.internal  # The model is under active development and its methods may change.
    def setup_method(self, method):
        Utils.initialize_model_parallel(1, 1)
        model_parallel_cuda_manual_seed(123)
...
class TestMiniRetroModelTokenParallel:

    def _init_mini_retro_model(self, cp_size, tp_size, sequence_parallel):
    ...
    def test_process_embedding_token_parallel(self, cp_size, tp_size, sequence_parallel, padding):
        ...
        model = None
        with ctx:
            model = self._init_mini_retro_model(cp_size, tp_size, sequence_parallel)
...
def test_mini_retro_model_parallelism(dtp, dpp, etp, epp):
    """
    ...
    language_config.language_model_type = "dummy"
    vision_config.vision_model_type = "clip"
    non_parallel_model = MiniRetroModel(
        language_transformer_config=language_config,
        language_transformer_layer_spec=language_layer_spec,
        language_vocab_size=8192,
        language_max_sequence_length=4096,
