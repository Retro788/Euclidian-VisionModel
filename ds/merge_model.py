from mini_retroonevision1_5.configuration_mini_retroonevision1_5 import MiniRetroonevision1_5Config
from mini_retroonevision1_5.modeling_mini_retroonevision1_5 import MiniRetroOneVision1_5_ForConditionalGeneration
...
def load_empty_model(llm_path):
    ...
    mini_retro_ov_config = MiniRetroonevision1_5Config()
    llm_config = AutoConfig.from_pretrained(llm_path, trust_remote_code=True)
    mini_retro_ov_config.text_config.update(llm_config.to_dict())
    mini_retro_ov_config.vision_config.text_hidden_size = mini_retro_ov_config.text_config.hidden_size
    model = MiniRetroOneVision1_5_ForConditionalGeneration(mini_retro_ov_config)
    return model, processor, tokenizer

def load_vit_weights(model, vit_path):
    """
    Load ViT weights and copy them to the vision part of MiniRetroOneVision1_5_ForConditionalGeneration
    
    Args:
        model: MiniRetroOneVision1_5_ForConditionalGeneration
...
def load_adapter_weights(model, adapter_path, cur_len):
    """
    Load Adapter weights and copy them to the corresponding part of MiniRetroOneVision1_5_ForConditionalGeneration
    
    Args:
        model: MiniRetroOneVision1_5_ForConditionalGeneration model
...
def validate_vit_consistency(model, vit_path, img_path):
    ...
    Args:
        model: MiniRetroOneVision1_5_ForConditionalGeneration after merged
...
def validate_llm_consistency(model, llm_path, sample_text):
    ...
    Args:
        model: Merged MiniRetroOneVision1_5_ForConditionalGeneration model
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_path", type=str, default="./checkpoints/merged/MiniRetro-OneVision-1.5-4B-stage0", help="Path to save the merged model")
