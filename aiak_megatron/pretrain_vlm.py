...
from megatron.core.models.multimodal.mini_retro_model import MiniRetroModel, DEFAULT_IMAGE_TOKEN_INDEX
from megatron.core.models.multimodal.mini_retro_spec import (
...
def model_provider(
    ...
    Note: currently, only MiniRetro model is supported. Follow-up changes will make this configurable.
    ...
    Returns:
        model (megatron.core.models.multimodal.mini_retro_model.MiniRetroModel): A multimodal model
    """
    args = get_args()
...
def train_valid_test_datasets_provider(train_val_test_num_samples):
    ...
    config = MultimodalDatasetConfig(
        ...
        image_h=args.img_h,
        image_w=args.img_w,
        preprocess_func=_preprocess_data_for_mini_retro,
    )
...
def _preprocess_data_for_mini_retro(data):
    """Preprocess data sample to the format expected by a MiniRetro model.

    Note: This doesn't support all the different modes in the official MiniRetro repo yet.
...
def forward_step(data_iterator, model: MiniRetroModel):
    """Forward training step.

    Args:
        data_iterator: Iterable dataset.
        model (megatron.core.models.multimodal.mini_retro_model.MiniRetroModel): Multimodal model
...
def mini_retro_embedding_ranks(pp_ranks):
    """MiniRetro's embedding ranks consist of the decoder's first and last ranks (ie, the ViT has no embeddings).
    Args:
...
def mini_retro_position_embedding_ranks(pp_ranks):
    """MiniRetro's embedding ranks consist of the singular rank of the model or the decoder's first rank.
    Args:
        pp_ranks: A list of global ranks that constitute a pipeline group.
    """
    args = get_args()
...
if __name__ == "__main__":
    ...
    pretrain(
        ...
        get_embedding_ranks=mini_retro_embedding_ranks,
        get_position_embedding_ranks=mini_retro_position_embedding_ranks,
    )